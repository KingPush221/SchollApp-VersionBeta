...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:#already_registred
            ...
            if role == 'admin':
                return redirect('portAdmin.html')
                ...
                return redirect('erreur! veillez choisir un role!')
        else:
...
def genie_informatique(request):
    return render(request,'genie-informatique.htm',)

def genie_civil(request):
    return render(request,'genie-civil.htm',)
...
def gestion_notes(request):
    if request.method == 'POST':
        ...
        form = NoteForm()
    return render(request, 'gestion_notes.htm', {'form': form})
...
def gestion_emploi_du_temps(request):
    ...
    return render(request, 'gestion_emploi_du_temps.htm', {'form': form, 'filieres': filieres, 'annees': annees, 'message': message})
...
def consulter_emploi(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "consulterEmploi.htm", {"form": form, "emploi": emploi})
    else:
        form = ConsulterEmploiForm()
    return render(request, "consulterEmploi.htm", {"form": form})
...
def consulter_notes(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        return render(request, 'consulterNotes.htm', context)
    else:
        form = ConsulterNotesForm()
    return render(request, "consulterNotes.htm", {"form": form})
...
def reinitialiser_emploi(request, filiere_id, annee_id):
    ...
    return render(request, 'gestion_emploi_du_temps.htm', context)
...
def gestion_absences(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('gestion_absences.htm')
    else:
        form = AbsencesForm()
    return render(request, 'gestion_absences.htm', {'form': form})   
...
def consulter_absences(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        return render(request, 'consulter_absences.htm', context)
    else:
        form =ConsulterAbsencesForm()
    return render(request, "consulter_absences.htm", {"form": form})
...
def valider_annee(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'validation_annee.htm', context)
    else:
        form =  ValiderAnneeForm()
        return render(request, 'validation_annee.htm', {'form': form})